...
public class BattleCharacterEditorWindow : EditorWindow
{
    ...
    private void DrawTimelineSection()
    {
        ...
        using (new EditorGUILayout.VerticalScope(boxStyle))
        {
            // Timeline Config 로드/생성
            LoadOrCreateTimelineConfig();
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                var newAttackTimeline = EditorGUILayout.ObjectField(
                    timelineConfig.attack1Timeline,
                    typeof(TimelineDataSO),
                    false) as TimelineDataSO;

                if (newAttackTimeline != timelineConfig.attack1Timeline)
                {
                    timelineConfig.attack1Timeline = newAttackTimeline;
                }

                // Timeline Asset 드래그&드롭 -> 자동 변환
                var draggedTimeline = EditorGUILayout.ObjectField(
                    null,
                    typeof(TimelineAsset),
                    false,
                    GUILayout.Width(100)) as TimelineAsset;

                if (draggedTimeline != null)
                {
                    ConvertAndAssignTimeline(draggedTimeline, "Attack1");
                }

                if (GUILayout.Button("Convert", GUILayout.Width(60)))
                {
                    ConvertTimelineFromPath("Attack1");
                }
            }
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                var newActiveTimeline = EditorGUILayout.ObjectField(
                    timelineConfig.activeSkill1Timeline,
                    typeof(TimelineDataSO),
                    false) as TimelineDataSO;

                if (newActiveTimeline != timelineConfig.activeSkill1Timeline)
                {
                    timelineConfig.activeSkill1Timeline = newActiveTimeline;
                }

                var draggedActive = EditorGUILayout.ObjectField(
                    null,
                    typeof(TimelineAsset),
                    false,
                    GUILayout.Width(100)) as TimelineAsset;

                if (draggedActive != null)
                {
                    ConvertAndAssignTimeline(draggedActive, "ActiveSkill1");
                }

                if (GUILayout.Button("Convert", GUILayout.Width(60)))
                {
                    ConvertTimelineFromPath("ActiveSkill1");
                }
            }
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                var newPassiveTimeline = EditorGUILayout.ObjectField(
                    timelineConfig.passiveSkill1Timeline,
                    typeof(TimelineDataSO),
                    false) as TimelineDataSO;

                if (newPassiveTimeline != timelineConfig.passiveSkill1Timeline)
                {
                    timelineConfig.passiveSkill1Timeline = newPassiveTimeline;
                }

                var draggedPassive = EditorGUILayout.ObjectField(
                    null,
                    typeof(TimelineAsset),
                    false,
                    GUILayout.Width(100)) as TimelineAsset;

                if (draggedPassive != null)
                {
                    ConvertAndAssignTimeline(draggedPassive, "PassiveSkill1");
                }

                if (GUILayout.Button("Convert", GUILayout.Width(60)))
                {
                    ConvertTimelineFromPath("PassiveSkill1");
                }
            }

            if (EditorGUI.EndChangeCheck())
            {
                EditorUtility.SetDirty(timelineConfig);
                isDirty = true;
            }
    ...
    // Helper 메서드들
    private void LoadOrCreateTimelineConfig()
    {
        if (timelineConfig == null && selectedCharacter != null)
        {
            string configPath = $"Assets/Cosmos/ResourcesAddressable/ScriptableObjects/Timelines/{selectedCharacter.CharacterName}_TimelineConfig.asset";

            timelineConfig = AssetDatabase.LoadAssetAtPath<CharacterTimelineConfig>(configPath);

            if (timelineConfig == null)
            {
                timelineConfig = ScriptableObject.CreateInstance<CharacterTimelineConfig>();
                timelineConfig.characterId = selectedCharacter.CharacterId;
                timelineConfig.characterName = selectedCharacter.CharacterName;
                ...
                AssetDatabase.CreateAsset(timelineConfig, configPath);
                AssetDatabase.SaveAssets();
            }
        }
    }

    private void ConvertAndAssignTimeline(TimelineAsset timeline, string slotName)
    {
        if (timeline == null || timelineConfig == null) return;
        ...
        {
            case "Attack1":
                timelineConfig.attack1Timeline = timelineData;
                break;
            case "ActiveSkill1":
                timelineConfig.activeSkill1Timeline = timelineData;
                break;
            case "PassiveSkill1":
                timelineConfig.passiveSkill1Timeline = timelineData;
                break;
        }
    ...
    private void ConvertTimelineFromPath(string slotName)
    {
        ...
        if (!string.IsNullOrEmpty(path))
        {
            ...
            if (timeline != null)
            {
                ConvertAndAssignTimeline(timeline, slotName);
            }
        }
    }
