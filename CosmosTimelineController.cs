...
public class CosmosTimelineController : EditorWindow
{
    ...
    private void LoadCharacterTimelines()
    {
        ...
        if (System.IO.File.Exists(configPath))
        {
            ...
            {
                if (timelineConfig.attack1Timeline != null)
                    options.Add("Attack1");
                if (timelineConfig.activeSkill1Timeline != null)
                    options.Add("ActiveSkill1");
                if (timelineConfig.passiveSkill1Timeline != null)
                    options.Add("PassiveSkill1");
            }
        }

        // 개별 Timeline 파일도 검색 (Config가 없거나 불완전한 경우)
        string animPath = $"Assets/Cosmos/ResourcesAddressable/Characters/{selectedCharacter.CharacterName}/Animations";
        if (AssetDatabase.IsValidFolder(animPath))
        {
            string[] patterns = { "Attack1", "ActiveSkill1", "PassiveSkill1" };

            foreach (string pattern in patterns)
            {
                if (!options.Contains(pattern)) // 이미 추가되지 않은 경우만
                {
                    string timelinePath = $"{animPath}/{selectedCharacter.CharacterName}_{pattern}_Timeline_Data.asset";
                    if (System.IO.File.Exists(timelinePath))
                    {
                        ...
                        if (timeline != null)
                        {
                            switch (pattern)
                            {
                                case "Attack1":
                                    if (timelineConfig.attack1Timeline == null)
                                        timelineConfig.attack1Timeline = timeline;
                                    break;
                                case "ActiveSkill1":
                                    if (timelineConfig.activeSkill1Timeline == null)
                                        timelineConfig.activeSkill1Timeline = timeline;
                                    break;
                                case "PassiveSkill1":
                                    if (timelineConfig.passiveSkill1Timeline == null)
                                        timelineConfig.passiveSkill1Timeline = timeline;
                                    break;
                            }
                        }
                    }
    ...
    private void LoadSelectedTimeline()
    {
        ...
        if (timelineConfig != null)
        {
            currentTimeline = timelineName switch
            {
                "Attack1" => timelineConfig.attack1Timeline,
                "ActiveSkill1" => timelineConfig.activeSkill1Timeline,
                "PassiveSkill1" => timelineConfig.passiveSkill1Timeline,
                _ => null
            };
        }
