...
namespace BattleCharacterSystem
{
    ...
    public class BattleCharacterDataSO : ScriptableObject
    {
        ...
        // 클래스 내부 필드 추가 (Line 56 이후)
        [Header("===== Timeline 설정 =====")]
        [SerializeField] private TimelineSetting timelineSettings = new TimelineSetting();

        // TimelineSetting 프로퍼티
        public TimelineSetting TimelineSettings => timelineSettings;
        ...
        // 내부 클래스 정의 (BattleCharacterDataSO 클래스 내부 맨 아래)
        [System.Serializable]
        public class TimelineSetting
        {
            ...
            {
                // 기존 CharacterTimelineConfig와 동일한 로직
                ...
                var custom = customTimelines.FirstOrDefault(x =>
                    x.stateName.Equals(stateName, StringComparison.OrdinalIgnoreCase));
